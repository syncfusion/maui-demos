...
namespace SampleBrowser.Maui.Scheduler.SfScheduler
{
    ...
    public class SchedulerDataBindingViewModel
    {
        ...
        /// <summary>
        /// Get or sets the drag and drop appointments. 
        /// </summary>
        public ObservableCollection<Meeting>? DragEvents { get; set; }
        ...
        {
            this.DragEvents = new ObservableCollection<Meeting>();
            Random randomTime = new();
            List<Point> randomTimeCollection = this.GettingTimeRanges();
            ...
            for (date = dateFrom; date < dateTo; date = date.AddDays(1))
            {
                for (int additionalAppointmentIndex = 0; additionalAppointmentIndex < 1; additionalAppointmentIndex++)
                {
                    ...
                    int hour = randomTime.Next((int)randomTimeCollection[additionalAppointmentIndex].X, (int)randomTimeCollection[additionalAppointmentIndex].Y);
                    meeting.From = new DateTime(date.Year, date.Month, date.Day, hour, 0, 0);
                    meeting.To = meeting.From.AddMinutes(30);
                    meeting.EventName = this.subjects[randomTime.Next(9)];
                    meeting.Background = this.colors[randomTime.Next(10)];
                    meeting.IsAllDay = false;
                    meeting.Notes = this.notes[randomTime.Next(10)];
                    meeting.StartTimeZone = TimeZoneInfo.Local;
                    meeting.EndTimeZone = TimeZoneInfo.Local;
                    this.DragEvents.Add(meeting);
                }
            }

            Meeting isAllDay = new Meeting();
            isAllDay.EventName = "All Day";
            isAllDay.Background = this.colors[randomTime.Next(5)];
            isAllDay.IsAllDay = true;
            isAllDay.Notes = this.notes[randomTime.Next(5)];
            isAllDay.StartTimeZone = TimeZoneInfo.Local;
            isAllDay.EndTimeZone = TimeZoneInfo.Local;
            this.DragEvents.Add(isAllDay);

            Meeting spanMeeting = new Meeting();
            spanMeeting.From = DateTime.Now.AddDays(-2);
            spanMeeting.To = spanMeeting.From.AddDays(1).AddHours(1);
            spanMeeting.EventName = "Spanned appointment";
            spanMeeting.Background = this.colors[randomTime.Next(3)];
            spanMeeting.IsAllDay = false;
            spanMeeting.StartTimeZone = TimeZoneInfo.Local;
            spanMeeting.EndTimeZone = TimeZoneInfo.Local;
            this.DragEvents.Add(spanMeeting);
        }
