...
namespace SampleBrowser.Maui.Scheduler.SfScheduler
{
    ...
    /// </summary>
    public class MonthInlineViewViewModel : INotifyPropertyChanged
    {
        #region Fields
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MonthInlineViewViewModel" /> class.
        /// </summary>
        public MonthInlineViewViewModel()
        {
        ...
        private void IntializeAppoitments()
        {
            ...
            DateTime dateFrom = DateTime.Now.AddMonths(-2);
            DateTime dateTo = DateTime.Now.AddMonths(2);

            for (DateTime monthStart = dateFrom; monthStart < dateTo; monthStart = monthStart.AddMonths(1))
            {
                int daysInMonth = DateTime.DaysInMonth(monthStart.Year, monthStart.Month);

                // Preselect up to 10 skip days for this month
                HashSet<int> skipDays = new HashSet<int>();
                while (skipDays.Count < 10)
                {
                    skipDays.Add(random.Next(1, daysInMonth + 1));
                }
                ...
                for (int day = 1; day <= daysInMonth; day++)
                {
                    ...
                    // Random number of appointments (between 3 and 7)
                    int appointmentCount = random.Next(3, 8);

                    for (int i = 0; i < appointmentCount; i++)
                    {
                        var meeting = new Meeting();
                        var randomTimeIndex = random.Next(randomTimeCollection.Count);
                        int hour = random.Next((int)randomTimeCollection[randomTimeIndex].X,
                                               (int)randomTimeCollection[randomTimeIndex].Y);

                        meeting.From = new DateTime(date.Year, date.Month, date.Day, hour, 0, 0);
                        meeting.To = meeting.From.AddHours(1);
                        meeting.EventName = this.subjects[random.Next(this.subjects.Count)];
                        meeting.Background = this.colors[random.Next(this.colors.Count)];
                        meeting.IsAllDay = false;
                        meeting.Notes = this.notes[random.Next(this.notes.Count)];
