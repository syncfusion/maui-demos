...
namespace SampleBrowser.Maui.Pdf.Pdf
{
    public partial class CompressExistingPDF : SampleView
    {
        ...
        private async void OnButtonClicked(object sender, EventArgs e)
        {
            ...
            bool result = await App.Current!.MainPage!.DisplayAlert("PDF is now " + (100 - (int)((outputSize / inputSize) * 100)).ToString() + "% smaller!", ((int)inputSize) + " KB -> " + ((int)outputSize) + " KB", "View PDF" , "Close");
            if (result)
            {
                //Saves the memory stream as file.
