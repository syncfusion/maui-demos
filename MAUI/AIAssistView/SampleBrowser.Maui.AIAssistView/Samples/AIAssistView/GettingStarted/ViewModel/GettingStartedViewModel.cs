...
namespace SampleBrowser.Maui.AIAssistView.SfAIAssistView
{
    public class GettingStartedViewModel : INotifyPropertyChanged
    {
        ...
        private bool isSendButttonVisible;
        private bool isListening;
        private ISfSpeechToText speechToTextService = Application.Current?.Handler?.MauiContext?.Services.GetService(typeof(ISfSpeechToText)) as ISfSpeechToText;
        ...
        public GettingStartedViewModel()
        {
            ...
            this.StartListeningCommand = new Command(ExecuteStartListeningCommand);
        ...
        {
            get { return isSendButttonVisible; }
            set { isSendButttonVisible = value; RaisePropertyChanged(nameof(IsSendButtonVisible)); }
        }
        ...
        private void InitializeConversationHistory()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                this.AutoSuggestions.Add(new AssistSuggestion { Text = title });
            }
        ...
        {
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "What is time management" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "How to improve productivity" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "What are communication skills" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "How to build a strong team" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "What is project management" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "How to handle stress" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "What is effective problem solving", });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "How to make better decisions" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "What is work-life balance" });
            this.AutoSuggestions.Add(new AssistSuggestion() { Text = "How to achieve professional growth", });
        ...
        private async void ExecuteStartListeningCommand()
        {
            if (this.speechToTextService != null && !this.IsListening)
            {
                this.IsListening = true;
                var isAuthorized = await speechToTextService.RequestAudioPermissionAsync();
                string recognitionText = string.Empty;
                if (isAuthorized)
                {
                    try
                    {
                        recognitionText = await speechToTextService.StartRecognitionAsync(CultureInfo.CurrentCulture,
                            new Progress<string>(partialText =>
                            {
                                recognitionText += partialText + " ";

                                RaisePropertyChanged(nameof(recognitionText));
                            }), tokenSource.Token);
