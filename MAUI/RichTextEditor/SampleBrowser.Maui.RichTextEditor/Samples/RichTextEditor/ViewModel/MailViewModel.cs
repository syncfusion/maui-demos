...
namespace SampleBrowser.Maui.RichTextEditor.RichTextEditor
{
    ...
    public class MailViewModel : INotifyPropertyChanged
    {
        ...
        private string _subject = "Follow-up on MAUI App Development";
        ...
        public MailViewModel(SfPopup? popup = null)
        {
            ...
            SelectedToAddresses.CollectionChanged += (_, __) => UpdateSendButtonState();
            SelectedFromAddresses.CollectionChanged += (_, __) => UpdateSendButtonState();
        ...
        /// <summary>
        /// Ensures the send button visibility.
        /// </summary>
        private void UpdateSendButtonState()
        {
            SendButtonVisibility = SelectedFromAddresses.Any() && SelectedToAddresses.Any() && !string.IsNullOrWhiteSpace(Subject) && !string.IsNullOrWhiteSpace(MailContent);
        }
