    ...
    public class FilteringBehavior : Behavior<SampleView>
    {
        ...
        public void OnFilterOptionsChanged(object sender, EventArgs e)
        {
            ...
            if (newPicker.SelectedIndex >= 0)
            {
                ...
                if (this.filterText != null && this.filterText.Text != null)
                {
                    this.OnFilterChanged();
                }
            }
        ...
        protected override void OnAttachedTo(SampleView bindAble)
        {
            ...
            this.filterText = bindAble.FindByName<Entry>("searchEntry");

            this.columnsList.SelectedIndex = 0;
            this.viewModel.Filtertextchanged = this.OnFilterChanged;

            if (this.filterText is Entry entry)
            {
        ...
        private void Entry_HandlerChanged(object? sender, EventArgs e)
        {
            if (filterText == null)
                return;
#if ANDROID
        var nativeView = filterText.Handler?.PlatformView as Android.Widget.EditText;
        var resource = Android.App.Application.Context.Resources;
        if (nativeView != null && resource != null && resource.DisplayMetrics != null)
        {
            nativeView.BackgroundTintList = ColorStateList.ValueOf(Colors.Transparent.ToPlatform());
            var density = resource.DisplayMetrics.Density;
            nativeView.SetPaddingRelative((int)(10 * density), 0, 0, 0);
            nativeView = null;
        }
#elif WINDOWS
            var nativeView = filterText.Handler?.PlatformView as Microsoft.UI.Xaml.Controls.TextBox;
            if (nativeView != null)
            {
                nativeView.BorderThickness = new Microsoft.UI.Xaml.Thickness(0);
                nativeView.Resources["TextControlBorderThemeThicknessFocused"] = new Microsoft.UI.Xaml.Thickness(0);
                nativeView.Resources["TextControlBackground"] = new Microsoft.UI.Xaml.Media.SolidColorBrush(Microsoft.UI.Colors.Transparent);
                nativeView.Resources["TextControlBackgroundPointerOver"] = new Microsoft.UI.Xaml.Media.SolidColorBrush(Microsoft.UI.Colors.Transparent);
                nativeView.Resources["TextControlBackgroundFocused"] = new Microsoft.UI.Xaml.Media.SolidColorBrush(Microsoft.UI.Colors.Transparent);

                nativeView = null;
            }
#elif MACCATALYST || IOS
            var nativeView = filterText.Handler?.PlatformView as UIKit.UITextField;
            if (nativeView != null)
            {
                nativeView.BorderStyle = UIKit.UITextBorderStyle.None;

                 // Sets left padding to the entry.
                nativeView.LeftViewMode = UIKit.UITextFieldViewMode.Always;
                nativeView.LeftView = new UIKit.UIView(new CoreGraphics.CGRect(0, 0, 10 , 0));
                nativeView = null;
