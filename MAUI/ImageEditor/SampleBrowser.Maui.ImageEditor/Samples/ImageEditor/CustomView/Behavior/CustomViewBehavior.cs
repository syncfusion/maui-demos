...
public class CustomViewBehavior : Behavior<SampleView>
{
    ...
    /// <summary>
    /// Holds the bottom toolbar instance.
    /// </summary>
    private ImageEditorToolbar? bottomToolbar;
    ...
    private ImageEditorToolbar CreateTopToolbar()
    {
        ...
        if (this.imageEditor != null)
        {
            ...
            var itemsToAdd = new[]
            {
                ...
                new { Item = saveGroup.Items.FirstOrDefault(i => i.Name == "SaveEdit"), Text = "Save edit", },
            };
    ...
    {
        bottomToolbar = new ImageEditorToolbar();
    ...
    private void OnImageEditorToolbarItemSelected(object? sender, ToolbarItemSelectedEventArgs e)
    {
        ...
        {
            ImageEditorToolbarItem? saveEditItem = this.imageEditor.Toolbars[0].ToolbarItems.OfType<ImageEditorToolbarItem>().FirstOrDefault(i => i.Name == "SaveEdit");
            if (saveEditItem != null && !saveEditItem.IsVisible)
            {
                this.SetSaveEditToolbarItemVisibility(true);
            }
            
            if (this.IsReplace || this.imageEditor.HasUnsavedEdits)
            {
        ...
        else if (e.ToolbarItem.Name == "SaveEdit")
        {
            this.SetSaveEditToolbarItemVisibility(false);
            return;
        }
    ...
    /// <summary>
    /// Method to show/hide the visibility of SaveEdit toolbar item and other items in the header toolbar.
    /// </summary>
    /// <param name="isVisible">The toolbar item visibility.</param>
    private void SetSaveEditToolbarItemVisibility(bool isVisible)
    {
        if (this.imageEditor == null || this.imageEditor.Toolbars == null)
        {
            return;
        }

        ImageEditorToolbar headerToolbar = this.imageEditor.Toolbars[0];
        foreach (var item in headerToolbar.ToolbarItems)
        {
            if (item.Name == "SaveEdit")
            {
                imageEditor.SetToolbarItemVisibility(item.Name.ToString(), isVisible);
            }
            else
            {
                imageEditor.SetToolbarItemVisibility(item.Name.ToString(), !isVisible);
    ...
    private void SetIconsEnableState(bool isEnable)
    {
        ...
        {
            foreach (ImageEditorToolbarItem item in this.bottomToolbar.ToolbarItems)
            {
                if (item.Name != "Add")
                {
