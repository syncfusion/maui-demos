...
public partial class ReadOnly : SampleView
{
    ...
    public ReadOnly()
    {
        ...
        DesktopToolbar.PropertyChanged += DesktopToolbar_PropertyChanged;
    ...
    {
        if (e.PropertyName == nameof(viewModel.ShowZoomLevel))
        {
            desktopZoomView.IsVisible = viewModel.ShowZoomLevel;
        }
    }

    private void DesktopToolbar_PropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (DesktopToolbar != null && DesktopToolbar.ZoomPercentage != null && DesktopToolbar.ZoomPercentage.IsPressed)
        {
            desktopZoomView.IsVisible = true;
        }
        else
        {
            desktopZoomView.IsVisible = false;
        }
    }
    ...
    {
        if (viewModel.ShowZoomLevel)
            viewModel.ShowZoomLevel = false;
    }
}