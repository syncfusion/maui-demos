...
public class ToolbarView : Microsoft.Maui.Controls.ContentView
{
    ...
    public Button? ZoomPercentage { get; set; }
    public Button? ZoomMobile { get; set; }
    ...
    void BindPdfViewerValues()
    {
        if (PdfViewer != null)
        {
            ...
            BindingBase zoomBindingBase = new Binding(nameof(Syncfusion.Maui.PdfViewer.SfPdfViewer.ZoomFactor), source: PdfViewer)
            {
                Converter = new ZoomPercentageConverter()
            };
            ZoomPercentage?.SetBinding(Button.TextProperty,zoomBindingBase);
