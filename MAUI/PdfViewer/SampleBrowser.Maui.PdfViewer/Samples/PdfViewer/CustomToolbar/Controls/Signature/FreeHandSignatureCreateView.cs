...
namespace SampleBrowser.Maui.PdfViewer.SfPdfViewer
{
    internal class FreeHandSignatureCreateView : SfView, ISignatureCreateView
    {
        ...
        {
            var inkPoints = signaturePad?.GetSignaturePoints() ?? new List<List<float>>();
            List<List<float>> copiedInkPoints = new List<List<float>>();
            foreach (List<float> inkPointList in inkPoints)
                copiedInkPoints.Add(new List<float>(inkPointList));
            ...
            return new DrawSignatureItem(selectedColor,copiedInkPoints, imageSource, inkImage);
        }
