...
namespace SampleBrowser.Maui.PdfViewer.SfPdfViewer
{
    internal class SignatureViewOverlay : SfView
    {
        ...
        private async void OnSignatureCreated(object? sender, EventArgs e)
        {
                ...
                if (fieldToSign != null)
                {
                    ...
                    if (SignatureHelper.CurrentSignatureItem is DrawSignatureItem drawSignature)
                    {
                        if (drawSignature.ImageSource is StreamImageSource streamImageSource)
                        {
                            ...
                            stream.Position = 0;
                            StampAnnotation stamp = new StampAnnotation(stream, pageNumber, new Rect(0, 0, imageWidth, imageHeight));
