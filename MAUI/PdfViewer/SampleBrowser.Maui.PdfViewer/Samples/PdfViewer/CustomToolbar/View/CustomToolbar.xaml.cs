...
public partial class CustomToolbar : SampleView
{
    ...
    public CustomToolbar()
    {
        ...
        SearchView.NoMoreMatchesFound += NoMoreMatchesFound;
        ...
        public void AddDocumentItems()
    {
        ...
        documentSeven.Children.Add(CreateView("Form PDF"));
...
#if !WINDOWS && !MACCATALYST
        PdfViewer_ModalViewDisappearing(this, EventArgs.Empty);
#endif
    }
...
# endif
    ...
    private void NoMoreMatchesFound(object? sender, EventArgs e)
    {
        MainThread.BeginInvokeOnMainThread(() => messageBox.Show("Search Result", "No more matches were found"));
    }
    ...
    private async void PdfViewer_Tapped(object? sender, GestureEventArgs? e)
    {
        ...
        if (this.BindingContext is CustomToolbarViewModel bindingContext)
        {
            ...
            //To disable thickness tool bar visibility
#if IOS || ANDROID
            bindingContext.IsThicknessToolbarVisible = false;
#endif
    ...
    {
        viewModel.CloseOverlaysAndPalettes();
        if (viewModel.SelectedAnnotation is FreeTextAnnotation)
        {
#if ANDROID || IOS
            viewModel.BottomToolbarContent = new AnnotationToolbar(viewModel);
