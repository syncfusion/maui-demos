...
namespace SampleBrowser.Maui.Kanban.SfKanban
{
    ...
    public partial class Customization : SampleView
    {
        ...
        /// <summary>
        /// Handles the drag start event of the kanban board to restirct card item removing from the source column on drag.
        /// </summary>
        /// <param name="sender">The kanban control.</param>
        /// <param name="e">The kanban drag start event args.</param>
        private void OnKanbanDragStart(object sender, KanbanDragStartEventArgs e)
        {
            if (e.Data is not MenuItem menuItem)
                return;

            if (menuItem.Category.ToString() == "Menu")
            {
                e.KeepCard = true;
            }
        }
