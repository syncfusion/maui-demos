...
namespace SampleBrowser.Maui.TreeView
{
    public class SfTreeViewGettingStartedBehavior : Behavior<SampleView>
    {
        ...
        protected override void OnAttachedTo(SampleView bindable)
        {
            ...
            entry = bindable.FindByName<SfEntry>("entry");
            if(entry != null)
            {
                entry.TextChanged += Editor_TextChanged;
            }
        ...
        {
            if(treeView != null && sender is SfEntry entry)
            {
                treeView.FilterText = entry.Text;
            }
        }
    }
}
