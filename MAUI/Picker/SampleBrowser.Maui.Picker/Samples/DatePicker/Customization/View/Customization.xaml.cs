...
public partial class Customization : SampleView
{
    ...
    {
        if (sender is Syncfusion.Maui.Picker.SfDatePicker picker && this.toDoDetails != null)
        {
            if (this.toDoDetails.Date.Date != picker.SelectedDate.Date)
            {
                this.toDoDetails.Date = picker.SelectedDate.Date;
            }
...
public class CustomPopUp : SfPopup
{
    ...
    {
        this.OnCreated?.Invoke(new ToDoDetails() { Date = this.picker.SelectedDate, Subject = this.entry.Text == string.Empty ? "No Title" : this.entry.Text}, new EventArgs());
        this.IsOpen = false;
    }
